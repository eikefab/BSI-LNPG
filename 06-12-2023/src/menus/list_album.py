...
def build(parent: Toplevel, window: Frame):
    ...
    def column_order():
        ...
        fill(window=window, widgets=table_widgets, data=controller.sort_albums(data, by.get(), descending=(descending.get() == 1)))
    ...
    def show_year_search():
        ...
        greater = Radiobutton(window, text="Maior que", variable=filter_year_type, value=1)
        lower = Radiobutton(window, text="Menor que", variable=filter_year_type, value=2)
    ...
    def filter_results():
        ...
        fill(window=window, data=controller.sort_albums(data, by.get(), descending=(descending.get() == 1)))

    descending_button = Checkbutton(window, text="Inverter", variable=descending, command=column_order)
    descending_button.grid(column=0, row=8, pady=(0, 20), sticky='W')

    filter = Entry(window, textvariable=filter_data)
    filter_label = Label(window, text="Procurar por:")

    filter_author = Radiobutton(window, text='Autor', variable=filter_type, value=0, command=hide_year_search)
    filter_year = Radiobutton(window, text='Ano', variable=filter_type, value=1, command=show_year_search)

    filter_action = Button(window, text="Filtrar", command=filter_results)

    filter_label.grid(column=0, row=0, padx=(0, 5), pady=(0, 20), sticky='W')
    filter_author.grid(column=0, row=1, pady=(5, 0), sticky='W')
    filter_year.grid(column=0, row=2, pady=(5, 0), sticky='W')
    filter_action.grid(column=1, row=6, pady=(15, 5))

    filter.grid(column=1, row=0, pady=(0, 20), padx=(0, 20))
